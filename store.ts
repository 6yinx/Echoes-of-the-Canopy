...
import { saveToDatabase, loadFromDatabase, getSessionId } from './services/saveService';
...
export const useGameStore = create<GameStore>((set, get) => ({
    ...
    saveGame: async () => {
        ...
        const saveData = {
            ...
            is_lantern_active: state.isLanternActive,
            inventory: state.inventory,
            active_slot: state.activeSlot,
            dropped_items: state.droppedItems,
            show_touch_controls: state.showTouchControls,
            is_auto_save: true
        };

        // Try database first
        const dbSuccess = await saveToDatabase(saveData);
        ...
        localStorage.setItem('echoes_save_v1', JSON.stringify(localSaveData));

        console.log(dbSuccess ? "💾 Game Saved to Cloud" : "💾 Game Saved Locally");
    },
    ...
    loadGame: async () => {
        ...
        if (dbData) {
            ...
            set({
                ...
                stamina: dbData.stamina,
                playerPosition: dbData.player_position,
                timeOfDay: dbData.time_of_day,
                isLanternActive: dbData.is_lantern_active,
                inventory: dbData.inventory || [],
                activeSlot: dbData.active_slot || 0,
                droppedItems: dbData.dropped_items || [],
                showTouchControls: dbData.show_touch_controls || false,
    ...
    hasSaveFile: async () => {
        ...
        // Fallback to localStorage
        return !!localStorage.getItem('echoes_save_v1');
    },
