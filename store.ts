...
import { GameState, NarrativeLog, MapLocation } from './types';
...
interface GameStore {
    ...
    // Map System
    setCurrentMap: (map: MapLocation) => void;
    getSpawnPosition: (map: MapLocation) => [number, number, number];
...
export const useGameStore = create<GameStore>((set, get) => ({
    ...
    setCurrentMap: (map) => {
        const spawnPos = get().getSpawnPosition(map);
        set({
            currentMap: map,
            playerPosition: spawnPos,
            teleportSignal: get().teleportSignal + 1,
    ...
    getSpawnPosition: (map) => {
        switch (map) {
            ...
            case MapLocation.OFFICE:
                return [0, 0.5, 0] as [number, number, number]; // Center of office
            default:
                return [0, 0.5, 0] as [number, number, number];
        }
    },
