...
const SceneContent = () => {
  // REMOVED: useState causing re-renders on every frame (playerPos)
    ...
    <>
      ...
      <Physics gravity={[0, -30, 0]} isPaused={isPaused || gameState === GameState.MENU}>
        {gameState === GameState.MENU ? (
          ...
          // Render player during LOADING so physics can settle on ground before view is shown
          // Removed onMove prop to prevent parent re-renders
          <Player />
        )}
        <Ground />
        <InfiniteWorld />
  ...
  useEffect(() => {
    ...
    if (gameState === GameState.PLAYING && !isPaused) {
      const autoSaveInterval = setInterval(() => {
        ...
        if (currentState === GameState.PLAYING && !currentPaused) {
          saveGame();
        }
